...
<!--
  ...
  Its /play-on-web/ scope isolates THIS page and also covers the engine
  assets at /play-on-web/2026-05-06/, so SharedArrayBuffer is available.

  The page UI below is copied from the engine's standalone index.html, but the
  heavy binaries (openlierox.js / .wasm / .data, ~25 MB) are NOT bundled in
  this repo — they are loaded from the same origin at /play-on-web/2026-05-06/
  via Module.locateFile and the async <script src> at the end.
-->
<script src="{{ '/play-on-web/coi-serviceworker.js' | relative_url }}"></script>

## Play OpenLieroX in your web browser

Runs entirely in your browser — no download or installation required. The game
needs to fetch about 25&nbsp;MB on first load, so give it a moment to start.
